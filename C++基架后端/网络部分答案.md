...
2. 包格式
2.1. TCP、UDP中的报文头有哪些字段
2.2. UDP的包有大小限制，是UDP协议自己的限制的吗？（是下层的IP协议的IP包大小限制）TCP没有限制吗？
2.3. 讲一讲TCP的flag字段有哪些？rst字段作用是什么？
3. TCP 连接管理
3.1. 三次握手；为什么需要第三次的ACK应答；
3.2. 四次挥手，各个状态，time_wait和close_wait在挥手的哪个阶段；四次挥手大量的time_wait状态是有哪些可能的原因
3.3. 第三次握手没有了会发生什么问题
3.4. TCP 为什么要三次握手，而不是两次握手
3.5. 四次挥手为什么多一次
3.6. 心跳探测
3.7. CLOSE_WAIT是什么？你说服务器上出现closewait，那么客户端上会出现closewait吗
4. 可靠数据传输
4.1. TCP为啥是可靠的？咋保证的呢？
4.2. UDP怎么在应用层做一些可靠性保证
5. 拥塞控制、流量控制
5.1. TCP拥塞控制过程
5.2. TCP流量控制
...
8.1. 追问三层路由：ping本地网络中的另一台主机，查询到一条/16的linklocal路由以后要干什么？回答：用ARP解析对端MAC，但是面试官不满意
8.2. 继续追问：发出ping的ICMP包后，内核是否会把/32路由插入本地路由表？回答：不会，只会插入二层neighbor表，面试官表示你确定？
...
1.1. 追问：DPDK驱动使用了用户态轮询的机制，和中断相比好处是什么